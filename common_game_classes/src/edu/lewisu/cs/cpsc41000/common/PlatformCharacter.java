...
public class PlatformCharacter extends MobileImageBasedScreenObject {
    ...
    public void setMaxVeritcalSpeed(float mvs) {
    ...
    int yorigin, int rotation, int scaleX, int scaleY, boolean flipX, 
    boolean flipY, float frameWidth, float frameHeight, int[] frameSequence,
    float animDelay, float gravity, float maxVerticalSpeed, float jumpSpeed, 
    ArrayList<ImageBasedScreenObject> platforms) {
        super(tex,xpos,ypos,xorigin,yorigin,rotation,scaleX,scaleY,flipX,flipY,
        frameWidth,frameHeight,frameSequence,animDelay);
        initMovement();
        setGravity(gravity);
        setMaxVeritcalSpeed(maxVerticalSpeed);
